...
export class PipelineService {
  ...
  async generateDiagram(prompt, options = {}) {
    ...
    try {
      ...
      let analysisData;
      let preGeneratedImages = [];

      if (typeof analysisResponse === 'object' && analysisResponse.toon && analysisResponse.images) {
        // Handle demo mode response
        analysisData = analysisResponse.toon;
        preGeneratedImages = analysisResponse.images;
      } else {
        analysisData = analysisResponse;
      }

      this.state.analysis = this.parseAnalysisResponse(analysisData);
      ...
      if (preGeneratedImages.length > 0) {
        images = preGeneratedImages;
        this.state.images = images;
        console.log('✅ Paso 2 completado - Usando imágenes pre-generadas (modo demo)');
      } else if (this.state.analysis.analysis?.needsImages) {
      ...
      const finalDiagram = await this.verifyAndOrganizeDiagram(this.state.analysis, images);
   ...
   */
  async verifyAndOrganizeDiagram(analysis, images) {
    try {
      ...
      // Llamar a Qwen 3 VL
      console.log('🔧 Llamando a this.chutes.verifyAndOrganize...');
      const result = await this.chutes.verifyAndOrganize(analysis, images);
